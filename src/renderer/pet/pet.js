...
(function() {
  ...
  async function init() {
    ...
    // ── Drag & Click ──
    var dragging = false, hasMoved = false, sx = 0, sy = 0, TH = 3;
    console.log('[Pet] Setting up drag/click on canvas:', canvas);
    canvas.addEventListener('mousedown', function(e) {
      console.log('[Pet] mousedown', e.screenX, e.screenY);
    ...
    window.addEventListener('mouseup', function() {
      console.log('[Pet] mouseup dragging=', dragging, 'hasMoved=', hasMoved);
      if (!dragging) return;
      dragging = false;
      if (hasMoved) {
        window.electronAPI.savePosition();
      } else {
        console.log('[Pet] toggleChatBar');
        toggleChatBar();
      }
    });
